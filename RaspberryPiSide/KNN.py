...
class KNN:
    ...
    def __init__(self):
        ...
        features = np.loadtxt(config.fpKNN + "features" + config.KNNVersion + ".txt", dtype=np.float32)
        labels = np.loadtxt(config.fpKNN + "labels" + config.KNNVersion + ".txt", dtype=np.float32)
