...
class KNN:
    ...
    def __init__(self):
        ...
        features = np.loadtxt(config.fpKNN + "features5.txt", dtype=np.float32)
        labels = np.loadtxt(config.fpKNN + "labels5.txt", dtype=np.float32)
    ...
    def classify(self, imgOutput):
        ...
        _, result, neighbors, distance = self.knn.findNearest(knnInput, 1)
